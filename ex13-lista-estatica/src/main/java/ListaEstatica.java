...
//    Se o vetor estiver “cheio”, exibe uma mensagem de “Lista cheia”
    public void adiciona(Integer elementoNovo) {
        if (nroElem != vetor.length) {
            vetor[nroElem++] = elementoNovo;
        } else {
...
//    Devolve true se removeu e false se não encontrou
    public boolean removeElemento(Integer elementoDigitado) {
        Integer indiceElementoDigitado = busca(elementoDigitado);

        if(indiceElementoDigitado >= 0){
            removePeloIndice(indiceElementoDigitado);
            return true;
        }
        return false;
    }
